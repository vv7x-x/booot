...
const handleMessage = async (sock, m) => {
    ...
    setTimeout(() => processedMessages.delete(msg.key.id), 10000);
    ...
    try {
        // Check for links - ignore if it's fromMe to prevent loops
        const links = text.match(urlRegex);
        if (links && !state && !isFromMe) {
            const url = links[0];
            console.log(`[LINK] From: ${sender} | URL: ${url}`);
...
const processDownload = async (sock, sender, url, type, quality, title, quoted) => {
    ...
    console.log(`[DOWNLOAD] Starting: ${type.toUpperCase()} | Quality: ${quality || 'Best'} | Title: ${title}`);
